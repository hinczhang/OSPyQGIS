...
from Dlg_SuperpixerSlic import Ui_Dialog
from PyQt5.QtCore import pyqtSignal
from PyQt5.QtWidgets import  QFileDialog, QMessageBox


class Func_SuperpixerSlic_class(QDialog, Ui_Dialog):
    mySignal = pyqtSignal(int)  # 规定信号
    def __init__(self, layers):
        print("init")
        super(Func_SuperpixerSlic_class, self).__init__()
        self.setupUi(self)
        # 连接函数
        self.slot_connect()
        # 状态变量、信息变量初始化
    ...
    def pushButton_ok_clicked(self):
        ...
        region_size = self.spinBox_region_size.value()
        ruler = self.doubleSpinBox_ruler.value()
        num = self.spinBox_num.value()
        print(num)
        slic = cv2.ximgproc.createSuperpixelSLIC(img, region_size=region_size, ruler=ruler)
        slic.iterate(num)  # 迭代次数，越大效果越好
        mask_slic = slic.getLabelContourMask()
        mask_inv_slic = cv2.bitwise_not(mask_slic)
        img_slic = cv2.bitwise_and(img, img, mask=mask_inv_slic)  # 在原图上绘制超像素边界
        self.create_tiff(fullpath, self.projections[index_r], self.geoTransform[index_r], img_slic)
