...
from Dlg_SuperpixerLsc import Ui_Dialog
from PyQt5.QtCore import pyqtSignal
from PyQt5.QtWidgets import  QFileDialog, QMessageBox


class Func_SuperpixerLsc_class(QDialog, Ui_Dialog):
    mySignal = pyqtSignal(int)  # 规定信号
    def __init__(self, layers):
        super(Func_SuperpixerLsc_class, self).__init__()
        self.setupUi(self)
        # 连接函数
        self.slot_connect()
        # 状态变量、信息变量初始化
    ...
    def pushButton_ok_clicked(self):
        ...
        lsc = cv2.ximgproc.createSuperpixelLSC(img, region_size, ratio)
        lsc.iterate(num)
        mask_lsc = lsc.getLabelContourMask()
        mask_inv_lsc = cv2.bitwise_not(mask_lsc)
        img_lsc = cv2.bitwise_and(img,img,mask = mask_inv_lsc)
        self.create_tiff(fullpath, self.projections[index_r], self.geoTransform[index_r], img_lsc)
